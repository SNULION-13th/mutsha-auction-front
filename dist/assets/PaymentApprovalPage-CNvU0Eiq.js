import { u as m, s as u, r, t as f, j as e, B as o } from "./index-BafdFBMt.js";
function j() {
  const s = m(),
    [a] = u(),
    t = r.useRef(!1),
    [n, c] = r.useState("pending"),
    { mutateAsync: d, error: x } = f();
  return (
    h(n === "success"),
    r.useEffect(() => {
      if (t.current) return;
      ((t.current = !0),
        (async () => {
          try {
            c("pending");
            const l = a.get("pg_token"),
              i = localStorage.getItem("tid");
            if (!l || !i) throw new Error("결제 정보가 올바르지 않습니다.");
            (await d({ pg_token: l, tid: i }), c("success"));
          } catch {
            c("error");
          }
        })());
...
const h = (s) => {
  ...
  r.useEffect(() => {
    ...
    const t = sessionStorage.getItem("returnTo") || "/";

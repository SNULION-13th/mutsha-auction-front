...
export type OrderResponse = {
  ...
  approved_at: string;
  canceled_at: string;
  selected_card_info: string;
};

export async function OrderTrack(): Promise<OrderResponse[] | null> {
  try {
    ...
    if (isAxiosError(e)) {
      console.error("orderTrack error:", e.response?.status, e.response?.data);
    } else {
      console.error("orderTrack unknown error:", e);
    }
    return null;
  }
}
